...
| 项 | 内容 |
|---|---|
| **症状** | 打印图片或测试页时报「操作无法完成（错误 **0x000003e3**）。I/O 操作已中止…」 |
| **根因** | 打印机队列用了通用的「**Microsoft IPP 类驱动程序**」，端口指向**链路本地 IPv6 地址**（`fe80::` 开头），这种地址会失效 |
| **怎么修** | 给队列换一个**稳定的 IPv4 地址端口**（走 9100）+ **HP 官方驱动** |
| **会复发吗** | 只换端口是**治标**——WSD 自动发现会抢回端口。要治本见 [「根治（方法三）」](#-换完端口又复发根治方法三) |

---

## 📍 第 0 步：查出打印机的 IP 地址

后面所有方法都需要先知道打印机在局域网里的 IPv4 地址（本文例子为 `192.168.31.123`，**请替换成你自己的**）。

**方法 A：打印机面板（最简单）**
按信息按钮（那个 `i` 字）→ 打印出一张纸 → 上面有 IP 地址。
...
## 🚀 方法二：PowerShell 命令（推荐，4 步）

本次实际使用的方法，一般**不需要管理员权限**（若提示拒绝访问，右键 PowerShell → 以管理员身份运行重试）。开始菜单搜索 `powershell` 回车打开。
...
> 端口名和 IP 都换成你自己的。标准 TCP/IP 端口默认走 9100，HP 网络打印机都支持。

### 第 2 步：把队列改成 HP 驱动 + 新端口

**先看一个坑**：自动生成的队列名带方括号（如 `HP Smart Tank 580-590 series [HPBD77C1]`），而 PowerShell 的打印命令会把 `-Name` 当**通配符**，带方括号的名字匹配不到。所以先用 WMI 精确匹配，把队列改成不带括号的名字：
...
> 如果重命名报错，说明队列名和你机器上的不一样。先运行 `Get-Printer` 查看实际名称，再把上面两行的名字替换成你的。
...
**为什么会复发**（一句话）：换完端口当时能打印，但 Windows 的**自动发现机制**还在——系统里有几个「软件设备」负责自动安装打印机（`SWD\IPP\...`、`SWD\DAFWSDPROVIDER\...`）。spooler 一重启或网络扫描一触发，它们就会**重新创建 WSD 端口并把队列端口抢回去**。

**需要管理员权限**。右键开始菜单 →「终端（管理员）」或「Windows PowerShell（管理员）」，再执行：
...
4. **只换过端口的话，复发风险仍在**：如果发现队列端口又变回 `WSD-*`（spooler 重启或重启电脑后），说明自动发现设备还在作祟，按上面的「根治（方法三）」做一次。
